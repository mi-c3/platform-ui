...
- Both pickers render in the v3 modal with 24-hour time (`ampm={false}`), an action bar of "Today"/"Cancel"/"OK", and `commitOn="change"` — this component snapshots both ends itself and restores them unless "OK" accepted the edit, so the picker publishes every selection instead of holding a draft of its own. The dialog's Save/Cancel/Clear are what commit the range.
- These are plain `DateTimePicker`s, i.e. MUI X v8's own responsive picker. They deliberately do **not** get the v3-style modal that [DateTimePickerRange](./DateTimePickerRange.md) restores for its "From"/"To" fields — pass `PickersFromProps`/`PickersToProps` if you want that look here.
