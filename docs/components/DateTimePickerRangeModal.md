...
- Both pickers render with `showTodayButton` and 24-hour time (`ampm={false}`) by default. `showTodayButton` sets the picker's action bar to exactly `['today']`, so the picker popper has no "Cancel"/"OK" of its own — the dialog's own Save/Cancel/Clear are the commit points.
