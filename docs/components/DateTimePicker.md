...
- Keyboard stepping in the field (<kbd>↑</kbd>/<kbd>↓</kbd>, <kbd>PageUp</kbd>/<kbd>PageDown</kbd>, <kbd>Home</kbd>/<kbd>End</kbd>) stops at `minDate`/`maxDate`/`minDateTime`/`maxDateTime`, which MUI X does not do on its own: it steps a section through that section's own boundaries only, so a 4-digit year walks to 9999 while the calendar stops at the default `maxDate` of 2099-12-31. A step that lands outside the range is dropped instead of published, leaving the field on the value it had. Typing is untouched — its intermediate values (year `0002` on the way to `2026`) still reach `onChange` and are still reported as invalid.
